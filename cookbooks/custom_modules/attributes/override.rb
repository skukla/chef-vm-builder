...
unless configured_custom_modules.nil?
    configured_custom_modules.each do |module_key, module_value|
        ...
        if configured_custom_modules[module_key].is_a? Chef::Node::ImmutableMash
            supported_settings.each do |setting|
                unless configured_custom_modules[module_key][setting].nil?
                    override[:custom_modules][:module_list][escaped_module_key][:settings][setting] = configured_custom_modules[module_key][setting]
                end
            end
        else
            unless configured_custom_modules[module_key].empty?
                override[:custom_modules][:module_list][escaped_module_key][:settings][:require] = configured_custom_modules[module_key]
            end
        end
        # Now we have the require statement value whether the setting is a hash or a string, so we can further parse it
        override[:custom_modules][:module_list][escaped_module_key][:settings][:vendor] = node[:custom_modules][:module_list][escaped_module_key][:settings][:require].split("/")[0]
        override[:custom_modules][:module_list][escaped_module_key][:settings][:name] = node[:custom_modules][:module_list][escaped_module_key][:settings][:require].split("/")[1]
        ...
        if configured_custom_modules[module_key]["configuration"].is_a? Chef::Node::ImmutableMash
            ...
            unless node[:custom_modules][:module_list][escaped_module_key][:settings][:require].nil?
                module_name = node[:custom_modules][:module_list][escaped_module_key][:settings][:name]
