...
magento_app "Clear the cron schedule" do
    ...
    only_if { 
        ::File.exist?("#{web_root}/app/etc/config.php") && ::File.exist?("/var/spool/cron/crontabs/#{user}") && build_action != "install"  
    }
end
...
composer "Create Magento #{family.capitalize} #{version} project" do
    ...
    only_if { 
        Dir.empty?("#{web_root}") && (build_action == "install" || build_action == "force_install") 
    }
end
...
composer "Set the project stability setting" do
    ...
    only_if { 
        ...
        (::File.exist?("#{web_root}/app/etc/config.php") && build_action != "install" && build_action != "reinstall") || 
...
composer "Update the project sort-packages setting" do
    ...
    only_if { 
        (::File.exist?("#{web_root}/composer.json")) || 
        (::File.exist?("#{web_root}/composer.json")) && (::File.exist?("#{web_root}/app/etc/config.php") && build_action != "install" && build_action != "reinstall") || 
        (::File.exist?("#{web_root}/composer.json")) && (!::File.exist?("#{web_root}/app/etc/config.php") && build_action != "reinstall") }
end

magento_app "Update the Magento version" do
    action :update_version
    only_if { 
        ::File.exist?("#{web_root}/composer.json") && ::File.foreach("#{web_root}/composer.json").grep(/#{version}/).any? && build_action == "update"  
    }
end
...
composer "Require the B2B modules" do
    ...
    options ["no-update"]
    only_if { 
        (family == "enterprise" && ::File.exist?("#{web_root}/composer.json") && !::File.foreach("#{web_root}/composer.json").grep(/extension-b2b/).any?) && (build_action == "force_install") || 
        (!::File.exist?("#{web_root}/app/etc/config.php") && build_action == "install") 
    }
end

if apply_patches && ((build_action == "force_install") || (apply_patches && build_action == "install" && !::File.exist?("#{web_root}/app/etc/config.php")))
    ...
    custom_modules.each do |custom_module_key, custom_module_data|
        custom_module "Add #{custom_module_data[:module_name]}" do
            ...
            not_if { 
                ...
                (build_action == "install" && (::File.exist?("#{web_root}/app/etc/config.php"))) || 
