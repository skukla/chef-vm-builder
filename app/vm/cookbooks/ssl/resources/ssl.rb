...
property :csr_configuration_file,
         String,
         default: node[:ssl][:csr_configuration_file]
property :csr_file, String, default: node[:ssl][:csr_file]
property :server_private_key_file,
         String,
         default: node[:ssl][:server_private_key_file]
property :server_extension_file,
         String,
         default: node[:ssl][:server_extension_file]
property :server_certificate_file,
         String,
         default: node[:ssl][:server_certificate_file]
property :chainfile, String, default: node[:ssl][:chainfile]
property :key_size, [String, Integer], default: node[:ssl][:key_size]
property :certificate_days,
         [String, Integer],
         default: node[:ssl][:certificate_days]
...
action :remove_ssl_files do
	execute 'Remove existing private key files' do
		...
		only_if do
			::File.exist?(
				"#{new_resource.ssl_directory}/#{new_resource.ca_private_key_file}",
			)
		end
	end
	...
	execute 'Remove existing certificate files' do
		...
		only_if do
			::File.exist?(
				"#{new_resource.ssl_directory}/#{new_resource.ca_certificate_file}",
			)
		end
	end
	...
	execute 'Remove existing serial files' do
		...
		only_if do
			::File.exist?(
				"#{new_resource.ssl_directory}/#{new_resource.ca_serial_file}",
			)
		end
	end
	...
	execute 'Remove existing CSR configuration files' do
		...
		only_if do
			::File.exist?(
				"#{new_resource.ssl_directory}/#{new_resource.csr_configuration_file}",
			)
		end
	end
	...
	execute 'Remove existing server private keys' do
		...
		only_if do
			::File.exist?(
				"#{new_resource.ssl_directory}/#{new_resource.server_private_key_file}",
			)
		end
	end
	...
	execute 'Remove existing server extension files' do
		...
		only_if do
			::File.exist?(
				"#{new_resource.ssl_directory}/#{new_resource.server_extension_file}",
			)
		end
	end
	...
	execute 'Remove existing server certificates' do
		...
		only_if do
			::File.exist?(
				"#{new_resource.ssl_directory}/#{new_resource.server_certificate_file}",
			)
		end
	end
end
...
action :generate_server_extension_file do
	vhost_urls_array = DemoStructureHelper.vm_urls
