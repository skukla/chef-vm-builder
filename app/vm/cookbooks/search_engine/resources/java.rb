...
property :package_name,
         String,
         default: node[:search_engine][:elasticsearch][:java][:package]
property :java_home,
         String,
         default: node[:search_engine][:elasticsearch][:java][:home]
property :environment_file,
         String,
         default: node[:search_engine][:elasticsearch][:java][:environment_file]
...
action :set_java_home do
	ruby_block 'Set JAVA_HOME for environment file' do
		block do
			StringReplaceHelper.set_java_home(
				new_resource.environment_file.to_s,
				new_resource.java_home.to_s,
			)
		end
