...
unsecure_base_url =
	node[:magento_restore][:magento][:settings][:unsecure_base_url]
secure_base_url = node[:magento_restore][:magento][:settings][:secure_base_url]
restore_path = node[:magento_restore][:restore_path]
backup_holding_area = node[:magento_restore][:holding_area]
apply_deploy_mode = node[:magento_restore][:magento][:build][:deploy_mode]
backup_version = node[:magento_restore][:remote_backup_data][:version]
use_secure_frontend =
	node[:magento_restore][:magento][:settings][:use_secure_frontend]
use_secure_admin =
	node[:magento_restore][:magento][:settings][:use_secure_admin]
...
if build_action == 'restore'
	...
	magento_cli 'Re-compile dependency injections and deploy static content' do
		...
		not_if do
			apply_deploy_mode || ::Dir.empty?(restore_path) || ::Dir.empty?(web_root)
		end
	end
