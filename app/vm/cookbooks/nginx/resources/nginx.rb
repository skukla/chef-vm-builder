...
property :ssl_private_key_file,
         String,
         default: node[:nginx][:ssl][:server_private_key_file]
property :ssl_certificate_file,
         String,
         default: node[:nginx][:ssl][:server_certificate_file]
property :use_secure_frontend,
         [String, Integer, TrueClass, FalseClass],
         default: node[:nginx][:magento][:settings][:use_secure_frontend]
property :use_secure_admin,
         [String, Integer, TrueClass, FalseClass],
...
action :configure_multisite do
	...
	vhost_entries = DemoStructureHelper.vhost_entries
		...
		variables(
			{
				...
				vhost_data: vhost_entries,
			},
		)
		only_if { ::Dir.exist?('/etc/nginx/sites-available/conf') }
	end

	vhost_entries.each do |vhost|
			...
			variables(
				{
					...
					use_secure_frontend:
						ValueHelper.process_value(new_resource.use_secure_frontend),
					use_secure_admin:
						ValueHelper.process_value(new_resource.use_secure_admin),
...
action :enable_multisite do
	link '/etc/nginx/sites-enabled/01-multisite.conf' do
		...
		group 'root'
		only_if do
			::File.exist?('/etc/nginx/sites-available/conf/01-multisite.conf')
		end
	end
end

action :set_permissions do
	%w[/etc/nginx /var/log/nginx].each do |directory|
