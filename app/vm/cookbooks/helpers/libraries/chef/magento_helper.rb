...
module MagentoHelper
  def self.get_consumer_list
    ...
    @community_consumer_list = Chef.node.default['magento']['build']['community_consumer_list']
    @enterprise_consumer_list = Chef.node.default['magento']['build']['enterprise_consumer_list']
    @user_consumer_list = Chef.node.override['magento']['build']['consumer_list']
    @consumer_list = []

    @family = if @family_override.nil? || @family_override.empty?
                @family_default
              else
                @family_override
              end
  ...
  def self.check_version(lower_bound, operator, upper_bound)
    Gem::Version.new(MagentoHelper.get_base_version(lower_bound)).send(operator, Gem::Version.new(upper_bound))
  end
end
