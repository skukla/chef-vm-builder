...
module DataPackHelperOld
	def self.data_pack_exists?(data_pack_source)
		@chef_files_path =
			Chef.node.default['magento_demo_builder']['chef_files']['path']
		...
		@chef_files.each do |entry|
			@entry_path =
				[File.dirname(entry), File.basename(entry)].join('/')
					.split('/')
					.pop(1)
					.join
	...
	def self.prepare_data_pack_names(package_name, repository_url)
		...
		if (repository_url.nil? || repository_url.empty?) &&
				repository_url.include?('github')
