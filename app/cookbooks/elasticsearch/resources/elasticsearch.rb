...
property :use,              [TrueClass, FalseClass],         default: node[:elasticsearch][:use]
...
action :install_app do
  execute "Add Elasticsearch #{new_resource.version} repository" do
    ...
    not_if { ::File.exist?("/etc/apt/sources.list.d/elastic-#{new_resource.version}.list") }
  end
...
action :uninstall do
  source_files = Dir.entries('/etc/apt/sources.list.d') - ['.', '..']
  old_version = source_files.find { |file| file.include?('elastic') && !file.include?("elastic-#{new_resource.version}.list") }

  if (!new_resource.use && ::File.exist?(new_resource.service_file)) || (!old_version.nil? && !old_version.include?(new_resource.version))
