  ...
  if ::File.exist?(composer_json)
    ...
    if build_action == 'update'
      magento_app 'Update the Magento version' do
        ...
        only_if { ::File.foreach(composer_json).grep(/#{version}/).none? }
      end
    end
    ...
    if %w[install force_install update].include?(build_action)
      ...
      magento_app 'Remove outdated modules' do
        ...
        only_if { ::File.foreach(composer_json).grep(/replace/).none? }
      end
      ...
      if family == 'enterprise'
        composer 'Require the B2B modules' do
          ...
          package_version '^1.0'
          options ['no-update']
          only_if { ::File.foreach(composer_json).grep(%r{magento/extension-b2b}).none? }
        end
      end
    end
  end
